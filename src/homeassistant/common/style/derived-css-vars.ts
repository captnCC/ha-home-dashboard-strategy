...
const _extractCssVars = (
  ...
  cssString.split(";").forEach((rawLine) => {
    ...
    if (line.startsWith("--") && condition(line)) {
      const [name, value] = line.split(":").map((part) => part.replaceAll("}", "").trim());
...
const extractVarFromBase = (
  ...
  if (baseVars[varName] && baseVars[varName].startsWith("var(")) {
    const baseVarName = baseVars[varName].substring(6, baseVars[varName].length - 1).trim();
 ...
 */
export const extractVar = (css: CSSResult, varName: string, baseVars?: Record<string, string>) => {
