...
export const generateEntityFilter = (
  ...
  const domains = filter.domain ? new Set(ensureArray(filter.domain)) : undefined;
  const hiddenDomains = filter.hidden_domains
    ? new Set(ensureArray(filter.hidden_domains))
    : undefined;
  const deviceClasses = filter.device_class ? new Set(ensureArray(filter.device_class)) : undefined;
...
export const findEntities = (entities: string[], filters: EntityFilterFunc[]): string[] => {
